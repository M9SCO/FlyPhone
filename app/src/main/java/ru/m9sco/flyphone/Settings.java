...
import ru.m9sco.flyphone.tools.InputFilterIP;
import ru.m9sco.flyphone.tools.InputFilterMinMax;
...
public class Settings extends AppCompatActivity {
    ...
    static public boolean Apply(Context context) {
        ...
        if (!SetDeviceBoolean(new String[]{"rRTLAGC", "rBIASTEE", "mBIASTEE"}, "ON", "OFF", context))
            return false;
        if (!SetDeviceInteger(new String[]{"mLINEARITY", "sGAIN"}, context))
            return false;

        if(!SetRTLbandwidth(context))
            return false;

        if (!SetUDPoutput("u1", context))
            return false;
        if (!SetUDPoutput("u2", context))
            return false;
        return true;
    }
    ...
    static private boolean SetDevice(String[] settings, Context context) {
        ...
        for (String s : settings) {
            ...
            if (FlyPhoneCatcher.applySetting(s.substring(0, 1), s.substring(1), p) != 0)
                return false;
        }
        return true;
    }
    ...
    static private boolean SetRTLbandwidth(Context context)
    {
        ...
        if(b) {
            if (FlyPhoneCatcher.applySetting("r", "BW", "192000") != 0)
                return false;
        }
        else {
            if (FlyPhoneCatcher.applySetting("r", "BW", "0") != 0)
                return false;
        }
        return true;
    }
    ...
    static private boolean SetDeviceBoolean(String[] settings, String st, String sf, Context context) {
        ...
        for (String s : settings) {
            ...
            if (FlyPhoneCatcher.applySetting(s.substring(0, 1), s.substring(1), b ? st : sf) != 0)
                return false;
        }
        return true;
    }
    ...
    static private boolean SetUDPoutput(String s, Context context) {
        ...
        if (b) {
            ...
            return FlyPhoneCatcher.createUDP(host, port) == 0;
